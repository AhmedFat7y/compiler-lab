...
def expand_terminals(term):
  term_copy = []
  for symbol, isVariable in term:
    if not isVariable:
      term_copy.extend([(c, isVariable) for c in symbol])
    else:
      term_copy.append((symbol, isVariable))
  return term_copy
...
def parse_term(term, heads):
  ...
  term_terminals_variables = expand_terminals(term_terminals_variables)
